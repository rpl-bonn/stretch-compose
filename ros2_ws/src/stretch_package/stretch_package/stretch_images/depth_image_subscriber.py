...
class DepthImageSubscriber(Node):
    ...
    def save_image(self):
        if self.rotate:
            image_path = '/home/ws/data/images/head_image_depth.png'
        else:
            image_path = '/home/ws/data/images/gripper_image_depth.png'
...
def main(args=None):
    ...
    node = DepthImageSubscriber('/camera/depth/image_rect_raw', rotate=True, save=True, vis=True)
    # node = DepthImageSubscriber('/gripper_camera/depth/image_rect_raw', rotate=False, save=True, vis=True)
    node.destroy_node()
    rclpy.shutdown()
