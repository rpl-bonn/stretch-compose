...
def execute_search (object_name: str, vis_block=False, save_block=True, no_proposals=3) -> bool:
    ...
    Args:
        object_name (str): Object to search for and grasp.
    ...
    audio_interface = AudioFeedbackInterface(node)
    ...
    try: 
        ...
        if object_name in graph_data["node_labels"]:
            in_scene_graph = True
            target_pos, furniture, front_normal, body_pose, furniture_id  = searchnet_planning.plan_furniture_search(object_name)
            checked_furniture_ids.append(furniture_id)
            print(f"{object_name} is in the scene graph. Searching for it in/on {furniture} at {target_pos}")
            audio_interface.send("remember_object_location", object=object_name, location=furniture)
            ...
            audio_interface.send("check_oject_existence", object=object_name, location=furniture)
            detected, detection_dict = yolo_detect_object(object_name, "head", save_block=save_block)
            if detected == False:
                print(f"Failed to detect {object_name} in/on {furniture} despite being in the scene graph.")
                oai = openai_client.oai_client
                img_path = os.path.join(IMG_DIR, f"camera_image_rgb.png")
                result = openai_client.check_image_response_for_object(oai, room_json_path, object_name, "gpt-4o-mini")
            
        else:     
            # Check if location proposals already exist for the object
            object_filename = f"{object_name.replace(' ', '_')}.json"
            ...
            if location_proposals_already_exist:
                print(f"Location proposals for {object_name} already exist. Skipping LLM location proposal step.")
                with open(object_location_file, "r") as loc_file:
                    loc_data = json.load(loc_file)
                    locations = loc_data.get("locations", [])
                    if len(locations) == 3:
                        print(f"Loaded 3 location proposals for {object_name}: {locations}")
                        audio_interface.send("likely_locations", object=object_name, locations_1=locations[0]["furniture_name"], locations_2=locations[1]["furniture_name"], locations_3=locations[2]["furniture_name"])
                    else:
                        print(f"Expected 3 locations, found {len(locations)} for {object_name}")
            
            else:
                candidate_found_with_high_probability = False
                print(f"{object_name} is NOT in the current scene graph. Switching to open vocabulary grasp search with clip and openmask3d")
                ovs = OpenVocabSearch()
                #print("Skipping open vocabulary search for now.")
                
                candidate_found, likely_furniture_id, likely_furniture_label, mask_sim, text_sim = ovs.search(object_name, no_proposals=no_proposals)
                
                if candidate_found:
                    probability = ovs.compute_probability(mask_sim, text_sim, 0.4)
                    
                if candidate_found and probability > 0.4:
                    print(f"Searching for {object_name} in/on {likely_furniture_label} (id: {likely_furniture_id}) with combined probability {probability:.3f}.")
                    result = ovs.result_to_json(object_name, likely_furniture_id, likely_furniture_label, probability, "unknown", object_location_json_path)
                    target_pos, furniture, front_normal, body_pose, furniture_id  = searchnet_planning.plan_furniture_search(object_name, 0)
                    checked_furniture_ids.append(furniture_id)
                    print(f"{object_name} is in the scene graph. Searching for it in/on {furniture} at {target_pos}")
        ...
        if detected == False and location_proposals_already_exist == False:          
            ...
            print(f"No suitable furniture found for {object_name} with high enough probability. Asking openai for likely locations.")
            oai = openai_client.oai_client
            result = openai_client.ask_for_shelf_with_room_json(oai, room_json_path, object_name, "trash can", "gpt-4o-mini")
        ...
        if (not candidate_found_with_high_probability or not detected) and location_proposals_already_exist:
            for i in range(no_proposals):
                target_pos, furniture, front_normal, body_pose, furniture_id  = searchnet_planning.plan_furniture_search(object_name, i)
                # Skip if already checked
                if furniture_id in checked_furniture_ids:
                    print(f"Already checked {furniture} ({furniture_id}).")
                    continue
                checked_furniture_ids.append(furniture_id)
                print(f"Searching for {object_name} in/on {furniture} ({target_pos}).")
                ...
                detected, detection_dict = yolo_detect_object(object_name, "head", save_block=save_block)
                if detected:
                    print(f"Found {object_name} in/on {furniture}: {detection_dict}")
                    break
                else:
                    print(f"Did not find {object_name} in/on {furniture}.")
        ...
        if detected:
            ...
            pcd_obj, pcd_env = collect_dynamic_point_cloud(object_name, joint_position_node, joint_pose_node, transform_node, start_pose, center, pcd)
                
            user_input = input("Enter Y/y to continue to grasping... And N/n to abort")
            if user_input.lower() == 'y':
                print("Proceeding to find new grasp dynamically and execute the grasp.")
                find_new_grasp_dynamically(joint_position_node, joint_pose_node, transform_node, body_pose, 0.15, 0.05, config, pcd_obj, pcd_env)
                success = True
            else:
                print("Aborting grasping as per user request.")
                stow_arm(stow_node)
                success = False
