...
from utils.sam3_detection_client import Sam3Client, call_sam3
from utils.time import convert_time
from utils.zero_shot_object_detection_sam3 import detect_drawer_handle_sam3, sam3_detect_object
...
def execute_search(OBJECT: str, OBJECT_DE: str) -> bool:
    ...
    try: 
        ...
        #     move_in_front_of(stow_node, base_node, head_node, joint_pose_node, body_pose, target_pos, 0.0, 0.0, 0.0, 0.0, stow=False, grasp=False)
        ...
        if not detected:
            ...
            for drawer_id in drawers_id_to_check:
                ...
                try:
                    ...
                    if handle_pose is None:
                        ...
                        #loop until handle is detected, as the drawer might be closed and the handle not visible at first
                        while handle_pose is None:
                            print(f"No handle detected for drawer {drawer_id}. Please open the drawer more.")
                            time.sleep(5.0)
                            rgb_img = get_rgb_picture(RGBImageSubscriber, joint_pose_node, "/gripper_camera/color/image_rect_raw", gripper=True)
                            depth_img = get_depth_picture(AlignedDepth2ColorSubscriber, joint_pose_node, "/gripper_camera/aligned_depth_to_color/image_raw", gripper=True)
                            handle_pose, drawer_type, _ = detect_drawer_handle_sam3(transform_node, depth_img, rgb_img, prompts, target_pos=drawer_center.coordinates)
                ...
                if drawer_type == "front":
                    ...
                    rgb_img = get_rgb_picture(RGBImageSubscriber, joint_pose_node, "/gripper_camera/color/image_rect_raw", gripper=True, save_block=SAVE_BLOCK, vis_block=VIS_BLOCK)
                    detected, detection_dict = sam3_detect_object(OBJECT, rgb_img, conf=0.2, save_block=SAVE_BLOCK)
...
def test_sam3_detection(obj: str, capture_image: bool = True):
    """
    Test SAM3 object detection.
    """
    saved_image_path = os.path.join(IMG_DIR, "gripper_image_rgb.png")
    rgb_img = None

    rclpy.init()
    try:
        if capture_image:
            print(f"Capturing gripper image for SAM3 test...")
            # Instantiate directly to avoid set_gripper call in get_rgb_picture
            image_node = RGBImageSubscriber("/gripper_camera/color/image_rect_raw", rotate=False, save=True, vis=VIS_BLOCK)
            rgb_img = image_node.cv_image
            image_node.destroy_node()

        if rgb_img is None:
            if not os.path.exists(saved_image_path):
                print(f"No saved image found at {saved_image_path}. Run with capture_image=True first.")
                return False, {}
            print(f"Reusing saved image at {saved_image_path}")
            rgb_img = cv2.imread(saved_image_path)

        print(f"Running SAM3 detection for '{obj}'...")
        detected, detection_dict = sam3_detect_object(obj, rgb_img, conf=0.2, save_block=SAVE_BLOCK)
        print(f"Detection result for {obj}: {detected}, {detection_dict}")
...
if __name__ == "__main__":
    ...
    # detected, detection_dict = test_sam3_detection(OBJECT)
    #test speacker on robot
    # say_on_robot("Found your bottle on the bookshelf. It is the blue one.")