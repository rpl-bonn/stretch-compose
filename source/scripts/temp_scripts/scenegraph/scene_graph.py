...
from scenegraph.graph_nodes import DrawerNode, ObjectNode
...
class SceneGraph:
    ...
    def scene_geometries(self, centroids: bool = True, connections: bool = True) -> list[tuple]:
        ...
        for node in self.nodes.values():
            if node.visible:
                ...
                if hasattr(node, "obb") and node.obb is not None:
                    obb = node.obb
                    obb.color = (0, 0, 1)  # Blue OBBs
                    geometries.append((obb, "obb_" + str(node.object_id), line_mat))
    ...
    def save_to_json(self, file_path) -> None:
        """ 
        Save SceneGraph as a JSON file. 
        """
        nodes = {
            "nodes": {
                idx: {
